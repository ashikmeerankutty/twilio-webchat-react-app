...
const sendUserMessage = (context, conversationSid, identity, messageBody) => {
    return context.getTwilioClient().conversations.conversations(conversationSid).messages.create({
        body: messageBody,
        author: identity,
...
exports.handler = async function (context, event, callback) {
    ...
    try {
        const result = await contactWebchatOrchestrator(formData, customerFriendlyName, context);
        ({ identity, conversationSid } = result);
    } catch (error) {
        return callback(null, sendResponse({ message: `Couldn't initiate WebChat: ${error?.message}` }, 400));
    }
